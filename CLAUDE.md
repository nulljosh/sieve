...
Renamed from Sieve to Siftbox 2026-09-11 — "Sieve" was taken on the App Store, and the standing rule is one name everywhere, not a Doorstock/BCGD-style split between the App Store listing and the product's own branding. Repo folder, GitHub (`nulljosh/siftbox`), domain (`siftbox.heyitsmejosh.com`), Xcode project/target/scheme (`Siftbox.xcodeproj`), Swift source filenames, entitlements filenames, bundle display name, and all copy were swept in the same pass. `sieve.heyitsmejosh.com` still resolves (same Worker, extra route) for anything that already links there — don't remove it without checking inbound links first. Bundle ID stays `com.nulljosh.sieve` (internal, invisible to users, changing it would mean new signing/profiles for no visible benefit).
...
- Outlook needs an Azure app registration (client ID + secret, redirect URI `https://siftbox.heyitsmejosh.com/auth/callback/outlook`) — blocked on Joshua, the code path is done but dead until secrets are set.
...
xcodebuild build -project Siftbox.xcodeproj -scheme Siftbox -destination 'platform=macOS'
xcodebuild build -project Siftbox.xcodeproj -scheme Siftbox -destination 'generic/platform=iOS Simulator'
```
